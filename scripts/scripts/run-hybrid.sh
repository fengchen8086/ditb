...
hbase_op_execute_script="./hybrid-server.sh"
parse_result_script="./parse-hybrid-results.sh"
...
data_dir_load="${data_dir_home}/ditb-load"
data_dir_run="${data_dir_home}/ditb-run"
...
cls_name_generate_data=ditb.hybrid.HybridDataGenerator

# workload_desc_path must be full path to be used in Java Program
cls_name_workload=ditb.workload.ExampleAWorkload
workload_desc_path="/home/winter/workspace/git/LCIndex-HBase-1.2.1/scripts/ditb/conf/workload-a-run"
...
fun_ExecuteOperations(){
    ...
    bash ${hbase_op_execute_script} ${cls_path} $1 $2 ${stat_file_name} ${client_hosts_file} $3 $4 ${cls_name_workload} ${workload_desc_path} $5
}
...
fun_CopyLogs(){
    ...
    echo "copy logs next"
	mkdir -p ${1}/remote-log-$6
    for host in `cat ${client_hosts_file}`; do
        scp -rq ${host}:"${remote_log_dir}/${host}-*-${3}-${4}-${5}.log" $1/remote-log-$6
...
fun_GeneLoadAndRun(){
    for host in `cat ${client_hosts_file}`; do
        ssh $host "rm -rf ${data_dir_load}"
        ssh $host "rm -rf ${data_dir_run}"
    done
    rm -rf ${data_dir_load} && rm -rf ${data_dir_run}
	workload_desc_path="/home/winter/workspace/git/LCIndex-HBase-1.2.1/scripts/ditb/conf/workload-a-load"
    fun_GenerateData ${data_dir_load} ${no_process} ${no_thread}
	workload_desc_path="/home/winter/workspace/git/LCIndex-HBase-1.2.1/scripts/ditb/conf/workload-a-run"
    fun_GenerateData ${data_dir_run} ${no_process} ${no_thread}
}

fun_Run() {
    now=`date +%Y%m%d-%H%M%S`
   # for index_type in NoIndex GSIndex CCIndex LCIndex IRIndex MDIndex; do
	for index_type in NoIndex; do
        ...
        bash ${hbase_op_script} kc && sleep 10 && start-dfs.sh && sleep 10 && start-hbase.sh && sleep 10
        # run insert
        fun_ExecuteOperations ${index_type} ${data_dir_load} ${no_process} ${no_thread} ${now} 2>&1 | tee ${cur_output}/load.log
        ...
        fun_CopyLogs ${cur_output} ${no_process} ${no_thread} ${now} ${index_type} load
		# start hbase to run scan
		start-hbase.sh && sleep 10
        fun_ExecuteOperations ${index_type} ${data_dir_run} ${no_process} ${no_thread} ${now} 2>&1 | tee ${cur_output}/execution.log
        # calculate storage cost
		#fun_ClearNonData
        #fun_CalStorageCost ${cur_output}/storage ${index_type}
        # save logs and copy to local
		stop-hbase.sh && sleep 10 
        fun_CopyLogs ${cur_output} ${no_process} ${no_thread} ${now} ${index_type} execution
    done
