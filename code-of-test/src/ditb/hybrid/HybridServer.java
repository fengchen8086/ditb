...
public class HybridServer {
  ...
  private List<HybridServerThread> threads;
  ...
  private void work(IndexType indexType, String dataDir, String statFileName, String clientFileName,
    ...
    do {
      Socket s = ss.accept();
      HybridServerThread th =
          new HybridServerThread(s, unFinishedClients, dataDir, param, nbThread, statFileName,
              syncObj);
      threads.add(th);
    ...
    startRemoteWork();
    System.out.println(
        "report put insert " + indexType + ", cost " + (System.currentTimeMillis() - start) / 1000.0
            + " to write on " + desc);
    setRemoteFlush();
    ...
    for (HybridServerThread t : threads) {
      t.join();
    }
    ss.close();
    System.out.println("server " + indexType + " finish");
    System.out.println(
        "report execution total " + indexType + ", cost " + (totalTime / 1000.0) + " to write on "
            + desc);
  }
   ...
   * @throws InterruptedException
   */
  private void startRemoteWork() throws InterruptedException {
    resetUnFinishedClients();
