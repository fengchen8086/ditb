...
public class PerfClient {
  ...
  private PerfInserterBase inserter;
  ...
  public void work(String serverName, String thisHost, int processId, int threadNumber)
    ...
    System.out.println("perf client connecting to " + serverName + ":" + PORT);
   ...
   * @throws IOException
   */
  private PerfInserterBase createInserter() throws IOException {
    if (IndexType.isMDPerf(indexType)) {
      return new PerfMD(workload.getTableName(), indexType, workload)
          .getPerfInserter(loadDataDir, processId, threadNum, statFilePath, reportQueue);
    } else {
  ...
  public static void main(String[] args)
    ...
    new PerfClient().work(args[0], args[1], Integer.valueOf(args[2]), Integer.valueOf(args[3]));
  }
}
