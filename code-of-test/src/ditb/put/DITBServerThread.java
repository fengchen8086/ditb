 ...
 */
public class DITBServerThread extends Thread {
  ...
  public DITBServerThread(Socket sock, List<String> unFinishedClients, String dataDir, String param,
  ...
  @Override public void run() {
    try {
        ...
        case START_TO_WORK:
          ...
          while (true) {
            ...
            if (DITBConstants.REMOTE_PUT_DONE_MSG.equals(msg)) { // ends with an read
              ...
              writeNoMeanMessage();
              System.out.println(
                  "[" + dateFormat.format(new Date()) + "] coffey from " + getClientProcessId()
                      + ": " + msg);
            } else {
      ...
      String msg = readShortMessage();
      System.out.println("coffey report " + getClientProcessId() + ": " + msg);
      writeNoMeanMessage();
      // receive report
      int times = Integer.valueOf(readShortMessage());
      writeNoMeanMessage();
      while (times > 0) {
        System.out.println("coffey report " + getClientProcessId() + ": " + readShortMessage());
        writeNoMeanMessage();
        --times;
      }
      readShortMessage();
