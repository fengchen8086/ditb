...
@InterfaceAudience.Private public class DefaultStoreFlusher extends StoreFlusher {
  ...
  @Override public List<Path> flushSnapshot(MemStoreSnapshot snapshot, long cacheFlushId,
    ...
    LOG.info("Flushed, sequenceid=" + cacheFlushId + ", memsize=" + StringUtils
        .humanReadableInt(snapshot.getSize()) + ", hasBloomFilter=" + writer.hasGeneralBloom()
  ...
  @Override public List<Path> flushSnapshot(MemStoreSnapshot snapshot, long cacheFlushId,
    ...
    if (store.getIndexType() == IndexType.NoIndex) {
      return flushSnapshot(snapshot, cacheFlushId, status);
    }
    ArrayList<Path> result = new ArrayList<Path>();
    int cellsCount = snapshot.getCellsCount();
    if (cellsCount == 0) return result; // don't flush if there are no entries
    ...
    try {
      ...
      synchronized (flushLock) {
        ...
        IOException e = null;
        try {
          performFlushWithIndex(scanner, writer, smallestReadPoint, store);
        } catch (IOException ioe) {
          e = ioe;
          // throw the exception out
          throw ioe;
