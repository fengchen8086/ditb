 ...
 * and a set of zero or more StoreFiles, which stretch backwards over time.
 */
...
public interface Store extends HeapSize, StoreConfigInformation,
   ...
   * top priority unless we have blocking compactions. (Pri <= 0)
   */int PRIORITY_USER = 1;
   ...
   * because the Region holds a write lock that will prevent any more reads or
   * writes.
   * 
   ...
   */
  List<KeyValueScanner> getScanners(boolean cacheBlocks, boolean isGet,
   ...
   * memstoreTS match the key &amp; memstoreTS value of the cell parameter.
   * 
   * @param cell
   */
  ...
  StoreFile.Writer createWriterInTmp(long maxKeyCount,
      Compression.Algorithm compression, boolean isCompaction,
      boolean includeMVCCReadpoint, boolean includesTags,
  ...
  CompactionContext requestCompaction(int priority,
      CompactionRequest baseRequest) throws IOException;

  CompactionContext requestCompaction(int priority,
      CompactionRequest baseRequest, User user) throws IOException;

  void cancelRequestedCompaction(CompactionContext compaction);

  /**
   * @deprecated see compact(CompactionContext, CompactionThroughputController,
   *             User)
   */
  @Deprecated
  ...
  List<StoreFile> compact(CompactionContext compaction,
      CompactionThroughputController throughputController, User user)
      throws IOException;
  ...
  /**
   * @return <tt>true</tt> if the store has any underlying reference files to
   *         older HFiles
   */
  ...
  /**
   * @return The amount of memory we could flush from this memstore; usually
   *         this is equal to {@link #getMemStoreSize()} unless we are carrying
   *         snapshots and then it will be the size of outstanding snapshots.
   */
   ...
   *         Writes older than this readPoint, are included in every read
   *         operation.
   */
  ...
  /**
   * @return The total amount of data processed during minor compactions, in
   *         bytes
   */
  ...
  /**
   * @return The total amount of data processed during major compactions, in
   *         bytes
   */
   ...
   * opened, and removes the store file readers for store files no longer
   ...
   * store. It should be greater than or equal to 0.0, any value greater than
   * 1.0 means we have too many store files.
   * <ul>
   ...
   * stripe separately and return the maximum value.
   * <p>
   * It is similar to {@link #getCompactPriority()} except that it is more
   * suitable to use in a linear formula.
   */
  double getCompactionPressure();

  /**
   * Replaces the store files that the store has with the given files. Mainly
   * used by secondary region replicas to keep up to date with the primary
   * region files.
   * 
   * @throws IOException
   */
