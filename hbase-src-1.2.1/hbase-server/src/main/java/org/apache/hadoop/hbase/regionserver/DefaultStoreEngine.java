 ...
 * derivatives.
 */
@InterfaceAudience.LimitedPrivate(HBaseInterfaceAudience.CONFIG) public class DefaultStoreEngine
    extends
  ...
  @Override public boolean needsCompaction(List<StoreFile> filesCompacting) {
  ...
  protected void createComponents(Configuration conf, Store store, KVComparator kvComparator)
    ...
    try {
      compactor = ReflectionUtils
          .instantiateWithCustomCtor(className, new Class[] { Configuration.class, Store.class },
  ...
  @Override public CompactionContext createCompaction() {
    return new DefaultCompactionContext();
  }

  public class DefaultCompactionContext extends CompactionContext {
    @Override public boolean select(List<StoreFile> filesCompacting, boolean isUserCompaction,
        boolean mayUseOffPeak, boolean forceMajor) throws IOException {
      request = compactionPolicy
          .selectCompaction(storeFileManager.getStorefiles(), filesCompacting, isUserCompaction,
              mayUseOffPeak, forceMajor);
      return request != null;
    }

    @Override public List<Path> compact(CompactionThroughputController throughputController)
        throws IOException {
    ...
    public List<Path> compact(CompactionThroughputController throughputController, User user)
        throws IOException {
     return compact(throughputController, user, null);
//      return compactor.compact(request, throughputController, user, null);
    }

    /**
     * in DefaultCompactionContext, lcindex need List<CompactJobQueue.CompactJob> to save compact job to relate it to the other succeed jobs.
     * but in other sub classes of CompactionContext, lcindex has not implemented it yet.
     * @throws IOException
     */
    @Override
    public List<Path> compact(CompactionThroughputController throughputController, User user,
        BasicJob[] jobs) throws IOException {
      return compactor.compact(request, throughputController, user, jobs);
    }

    @Override public List<StoreFile> preSelect(List<StoreFile> filesCompacting) {
      return compactionPolicy
          .preSelectCompactionForCoprocessor(storeFileManager.getStorefiles(), filesCompacting);
    }
  }
