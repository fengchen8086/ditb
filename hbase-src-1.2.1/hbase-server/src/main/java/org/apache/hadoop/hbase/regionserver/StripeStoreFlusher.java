...
public class StripeStoreFlusher extends StoreFlusher {
  ...
  public List<Path> flushSnapshot(MemStoreSnapshot snapshot, long cacheFlushSeqNum,
    ...
    try {
      ...
      StripeMultiFileWriter.WriterFactory factory =
          createWriterFactory(snapshot.getTimeRangeTracker(), cellsCount);
  ...
  private StripeMultiFileWriter.WriterFactory createWriterFactory(final TimeRangeTracker tracker,
      final long kvCount) {
    return new StripeMultiFileWriter.WriterFactory() {
      @Override
      public Writer createWriter() throws IOException {
        StoreFile.Writer writer =
            store.createWriterInTmp(kvCount, store.getFamily().getCompression(),
            /* isCompaction = */false,
  ...
  public static class SizeStripeFlushRequest extends StripeFlushRequest {
    ...
    @Override
    public StripeMultiFileWriter createWriter() throws IOException {
      return new StripeMultiFileWriter.SizeMultiWriter(this.targetCount, this.targetKvs, OPEN_KEY,
          OPEN_KEY);
    }
  }
  ...
  public List<Path> flushSnapshot(MemStoreSnapshot snapshot, long cacheFlushSeqNum,
    ...
    throw new IOException("LCTODO: flushSnapshot not implemented in " + this.getClass().getName());
  }
}
