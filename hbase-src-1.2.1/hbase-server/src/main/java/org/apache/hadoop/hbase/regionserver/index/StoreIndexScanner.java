...
public class StoreIndexScanner implements ChangedReadersObserver {
  ...
  public synchronized boolean nextFromMemStore(KeyValueScanner scanner, int interval)
    ...
    while (true) {
      ...
      Cell nextCell = scanner.next();
      if (nextCell == null) {
        return false;
      }
      if (!(nextCell instanceof KeyValue)) {
        throw new IOException("scanner.next() is not an instance of KeyValue, it is: "
            + nextCell.getClass().getName());
      }
      currentKV = (KeyValue) nextCell;
