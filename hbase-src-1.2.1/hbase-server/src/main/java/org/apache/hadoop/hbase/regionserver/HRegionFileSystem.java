...
@InterfaceAudience.Private public class HRegionFileSystem {
  ...
  public boolean hasReferences(final String familyName) throws IOException {
    FileStatus[] files =
        FSUtils.listStatus(fs, getStoreDir(familyName), new FSUtils.ReferenceFileFilter(fs));
  ...
  public Path commitStoreFile(final String familyName, final Path buildPath, final HStore store,
      boolean dueToFlush) throws IOException {
    return commitStoreFile(familyName, buildPath, -1, false, store, dueToFlush);
  }
  ...
  /**
   * Moves multiple store files to the relative region's family store directory. LCNOTE: this method
   * is never called!
   *
    ...
    if (splitPolicy == null || !splitPolicy.skipStoreFileRangeCheck(familyName)) {
      ...
      try {
        if (top) {
          ...
          if (f.getComparator().compareFlatKey(splitKey.getBuffer(), splitKey.getKeyOffset(),
              splitKey.getKeyLength(), lastKey, 0, lastKey.length) > 0) {
            return null;
          }
        } else {
          ...
          if (f.getComparator().compareFlatKey(splitKey.getBuffer(), splitKey.getKeyOffset(),
              splitKey.getKeyLength(), firstKey, 0, firstKey.length) < 0) {
            return null;
          }
        }
      } finally {
   ...
   * @throws IOException
   */
  Path mergeStoreFile(final HRegionInfo mergedRegion, final String familyName, final StoreFile f,
