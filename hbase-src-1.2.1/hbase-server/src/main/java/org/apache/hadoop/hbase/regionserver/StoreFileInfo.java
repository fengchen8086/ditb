...
@InterfaceAudience.Private public class StoreFileInfo {
  ...
  public final HFileLink link;
  // FileSystem handle
  private final FileSystem fs;
  private final Path initialPath;

  public Path irIndexPath = null;
  public Path irIndexReferencePath = null;
  public boolean hasIRIndex = false;

  public Path lmdDataPath = null;
  public Path lmdBucketPath = null;
  public boolean hasLMDIndex = false;
  ...
  public StoreFileInfo(final Configuration conf, final FileSystem fs, final Path initialPath)
    ...
    Path p = initialPath;
    // irindex
    Path tmpPath = getIRIndexPathFromPath(p);
    if (fs.exists(tmpPath)) {
      this.irIndexPath = tmpPath;
      this.hasIRIndex = true;
    }
    tmpPath = LMDIndexParameters.getTmpSecondaryFilePath(p);
    if (fs.exists(tmpPath)) {
      this.lmdDataPath = tmpPath;
      this.lmdBucketPath = LMDIndexParameters.getTmpBucketFilePath(p);
      this.hasLMDIndex = true;
    }
    if (fs instanceof LocalFileSystem && p.getParent().getName()
        .endsWith(LCIndexConstant.LCINDEX_DIR_NAME)) {
      this.reference = null;
      this.link = null;
    } else if (p.getName().endsWith(LMDIndexConstants.DATA_FILE_SUFFIX) || p.getName()
        .endsWith(LMDIndexConstants.BUCKET_FILE_SUFFIX)) {
      this.reference = null;
      this.link = null;
    } else if (HFileLink.isHFileLink(p)) {
      // the rest, as normal
      // HFileLink
      this.reference = null;
      this.link = HFileLink.buildFromHFileLinkPattern(conf, p);
      if (LOG.isTraceEnabled()) LOG.trace(p + " is a link");
      System.out.println("LCDBG, unimplemented for HFileLink.isHFileLink(p): " + p);
      ...
      // index
      tmpPath = getIRIndexPathFromPath(referencePath);
      if (fs.exists(tmpPath)) {
        this.irIndexReferencePath = tmpPath;
        this.hasIRIndex = true;
      }
    } else if (isHFile(p)) {
      // HFile
  ...
  public StoreFile.Reader open(final FileSystem fs, final CacheConfig cacheConf,
      final boolean canUseDropBehind) throws IOException {
    ...
    if (this.coprocessorHost != null) {
      reader = this.coprocessorHost
          .preStoreFileReaderOpen(fs, this.getPath(), in, length, cacheConf, reference);
    }
    if (reader == null) {
    ...
    if (this.coprocessorHost != null) {
      reader = this.coprocessorHost
          .postStoreFileReaderOpen(fs, this.getPath(), in, length, cacheConf, reference, reader);
    }
    return reader;
  }
  ...
  @Override public String toString() {
    return this.getPath() + (isReference() ?
        "-" + getReferredToFile(this.getPath()) + "-" + reference :
        "");
  }
