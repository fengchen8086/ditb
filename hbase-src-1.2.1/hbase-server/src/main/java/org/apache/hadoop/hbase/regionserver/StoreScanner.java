 ...
 */
@InterfaceAudience.Private public class StoreScanner extends NonReversedNonLazyKeyValueScanner
  ...
  public StoreScanner(Store store, ScanInfo scanInfo, Scan scan, final NavigableSet<byte[]> columns,
      long readPt, LMDIndexConstants constants) throws IOException {
    this(store, scan, scanInfo, columns, readPt, scan.getCacheBlocks());
    if (columns != null && scan.isRaw()) {
      throw new DoNotRetryIOException("Cannot specify any column for a raw scan");
    }
    ...
    List<KeyValueScanner> scanners =
        getLMDCIndexDirectScannersNoCompaction(ScanRange.ScanRangeList.getScanRangeList(scan));
    // Seek all scanners to the start of the Row (or if the exact matching row
    // key does not exist, then to the start of the next matching Row).
    // Always check bloom filter to optimize the top row seek for delete
    // family marker.
  ...
  // LCIndex scanner
  public StoreScanner(Store store, ScanInfo scanInfo, Scan scan, final NavigableSet<byte[]> columns,
      long readPt, ScanRange scanRange, LCIndexMemStoreScanner2 memScanner) throws IOException {
    this(store, scan, scanInfo, columns, readPt, scan.getCacheBlocks());
    if (columns != null && scan.isRaw()) {
      throw new DoNotRetryIOException("Cannot specify any column for a raw scan");
    }
    ...
    this.store.addChangedReaderObserver(this);
    // Pass columns to try to filter out unnecessary StoreFiles.
    List<KeyValueScanner> scanners = getLCIndexScannersNoCompaction(scanRange, readPt, memScanner);
    // Seek all scanners to the start of the Row (or if the exact matching row
    // key does not exist, then to the start of the next matching Row).
    // Always check bloom filter to optimize the top row seek for delete
    // family marker.
    for (KeyValueScanner scanner : scanners) {
      //      System.out.println(
      //          "winter before init StoreScanner, peek of scanner: " + indexedPeekToString(scanner)
      //              + ", scanner is: " + scanner.getClass().getName());
      scanner.seek(matcher.getStartKey());
      //      System.out.println(
      //          "winter after init StoreScanner, peek of scanner: " + indexedPeekToString(scanner)
      //              + ", scanner is: " + scanner.getClass().getName());
    }
    //    this.storeLimit = scan.getMaxResultsPerColumnFamily();
    //    this.storeOffset = scan.getRowOffsetPerColumnFamily();
    resetKVHeap(scanners,
        store.getComparator()); // same as heap = new KeyValueHeap(scanners, store.getComparator());
  }
  ...
  @VisibleForTesting StoreScanner(final Scan scan, ScanInfo scanInfo, ScanType scanType,
  ...
  protected List<KeyValueScanner> getScannersNoCompaction() throws IOException {
    ...
    return selectScannersFrom(store
        .getScanners(cacheBlocks, get, usePread, isCompaction, matcher, scan.getStartRow(),
            scan.getStopRow(), this.readPt));
  }

  private List<KeyValueScanner> getLCIndexScannersNoCompaction(ScanRange scanRange, long readPt,
      LCIndexMemStoreScanner2 memScanner) throws IOException {
    final boolean isCompaction = false;
    boolean usePread = get || scanUsePread;
    return selectScannersFrom(((HStore) store)
        .getLCIndexScanners(false, usePread, isCompaction, matcher, readPt, scanRange, memScanner));
  }

  protected List<KeyValueScanner> getLMDCIndexDirectScannersNoCompaction(
      ScanRange.ScanRangeList scanRangeList) throws IOException {
    final boolean isCompaction = false;
    return selectScannersFrom(((HStore) store)
        .getLMDIndexDirectScanners(cacheBlocks, false, false, isCompaction, matcher,
            scan.getStartRow(), scan.getStopRow(), this.readPt, scanRangeList));
  }
  ...
  @Override public boolean next(List<Cell> outResult) throws IOException {
  ...
  @Override public void updateReaders() throws IOException {
    lock.lock();
    try {
      if (this.closing) return;
  ...
  @Override public boolean reseek(Cell kv) throws IOException {
    lock.lock();
    try {
  ...
  public static String indexedPeekToString(KeyValueScanner scanner) {
    if (scanner == null) return "scanner is null";
    if (scanner.peek() == null) return "scanner.peek() is null";
    return String.valueOf(Bytes.toInt(scanner.peek().getRow()));
    //    return LCIndexConstant.mWinterIndexedtoStringWithBinary(scanner.peek());
  }
}
