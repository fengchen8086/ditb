...
public class IndexResultScanner {
  ...
  static class NoIndexSingleScanner extends SingleScanner {
    ...
    public NoIndexSingleScanner(Scan scan, byte[][] resultColumns, Table table) throws IOException {
      super(scan, resultColumns, table);
    }

    public void run() {
      LOG.debug(this.getName() + "-" + getId() + " start scanning, startkey:" + Bytes
          .toStringBinary(scan.getStartRow()) + ", endkey:" + Bytes
      ...
      while (!finished) {
        ...
        if (!finished) {
          ...
          LOG.debug(
              this.getName() + "-" + getId() + " finish scanning," + " total records:" + count);
        }
      }
  ...
  static class IndexSingleScanner extends SingleScanner {
    ...
    class GetThread extends Thread {
      ...
      public GetThread(FilterList ftlist, byte[][] resultColumns, Table table, byte[] rowkey) {
        this.ftlist = ftlist;
        this.resultColumns = resultColumns;
        this.table = table;
        this.rowkey = rowkey;
