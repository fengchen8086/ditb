...
public class IndexTableDescriptor {
  ...
  public IndexTableDescriptor(HTableDescriptor tableDescriptor) throws IOException {
    ...
    if (descriptor.getTableName().equals(TableName.META_TABLE_NAME)) {
      throw new IllegalArgumentException("Table name can't be " + TableName.META_TABLE_NAME);
    }
    this.readFromTable();
  }
  ...
  private void checkNewIndex(IndexSpecification indexSpec) {
    indexSpec.setTableName(this.descriptor.getTableName());
      ...
      if (indexSpec.getAdditionMap().size() != 0) {
        ...
        for (byte[] temp : indexSpec.getAdditionMap().keySet()) {
          if (!families.contains(temp)) {
            throw new IllegalArgumentException(
                "Error addition family " + Bytes.toString(temp) + " for index column " + Bytes
                    .toString(indexSpec.getIndexColumn()));
          }
        }
      }
    }
  ...
  public void addIndex(IndexSpecification index) throws IOException, IndexExistedException {
    ...
    System.out.println(
        "winter want to check index column name: " + Bytes.toString(index.getIndexColumn()));
  ...
  public boolean equals(Object args) {
    ...
    if (this.descriptor.equals(outDesc.descriptor) && this.indexSpecifications
        .equals(outDesc.indexSpecifications)) {
      return true;
    }
  ...
  //    IndexSpecification indexSpec = this.getIndexSpecification(indexColumn);
  //    return Bytes.add(indexSpec.getIndexTableName().getName(), IndexTable.CCT_FIX);
  //  }
}
