...
public class QueryCmd {
  ...
  public static void main(String[] args) throws Exception {
    ...
    options.addOption("i", "useIndex", true,
        "set it true if you want to use index when scanning, default is true.");
    options.addOption("g", "maxGetsPerScanner", true,
        "Set max get threads for a scanner, it will be used when SecIndex or ImpSecIndex is chose to scan and index table doesn't contain all result columns, default value is 10.");
    options.addOption("t", "maxScanners", true,
        "max number of threads to start to query, default value is 10.");
    options.addOption("p", "printResults", true,
        "set it true if you want to print result, default value is false.");
    options.addOption("n", "printInterval", true, "print progress interval, default is 100 rows.");
    options.addOption("c", "scannerCaching", true,
        "Sets the number of rows that a scanner will fetch at once, default is 'hbase.client.scanner.caching'.");
    ...
    try {
      ...
      IndexTable table = new IndexTable(conn, query.getTableName());
      if (cli.hasOption("i")) {
      ...
      while ((r = rs.next()) != null) {
        ...
        if (rs.getTookOutCount() % interval == 0) {
          String str =
              "------------results:" + rs.getTotalCount() + "," + "time:" + rs.getTotalScanTime()
                  + "ms," + "totalscanner:" + rs.getTotalScannerNum() + "," + "finishedscanner:"
      ...
      String str =
          "------------results:" + rs.getTotalCount() + "," + "time:" + rs.getTotalScanTime()
              + "ms," + "totalscanner:" + rs.getTotalScannerNum() + "," + "finishedscanner:" + rs
              .getFinishedScannerNum() + "-------------final";
      System.out.println(str);
