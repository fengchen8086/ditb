...
import org.apache.hadoop.hbase.index.ccindex.IndexResultScanner.IndexSingleScanner;
import org.apache.hadoop.hbase.index.ccindex.IndexResultScanner.NoIndexSingleScanner;
import org.apache.hadoop.hbase.index.ccindex.IndexResultScanner.SingleScanner;
...
public class IndexTable implements Closeable {
  ...
  public Map<byte[], Table> indexTableMaps = null;
  public Map<byte[], Table> cctTableMaps = null;
   ...
   * @throws IOException
   */
  public IndexTable(final Connection conn, final String tableName) throws IOException {
      ...
      for (Map.Entry<byte[], List<KeyValue>> entry : delete.getFamilyMap().entrySet()) {
        for (KeyValue kv : entry.getValue()) {
          if (kv.getQualifierLength() == 0) // delete family
          {
            ...
            if (indexSpec.getIndexType() == IndexType.UDGIndex) {
   ...
   * @throws Exception
   */
  public IndexResultScanner getScanner(IndexQuerySQL indexSql, int maxScanThreads)
      throws Exception {
  ...
  public void query(double c3_end, String c5_equal, boolean printProgress, boolean print)
    ...
    if (IndexType.CCIndex == this.indexDesc.getIndexSpecification(list.get(index).getColumn())
      ...
      Table table = this.indexTableMaps.get(list.get(index).getColumn());
      ...
      while ((result = scanner.next()) != null) {
        ...
        if (printProgress && (count % 100000 == 0)) {
          System.out.println(
              "Time elapsed: " + (System.currentTimeMillis() - startTime) + " ms, result count: "
                  + count);
        }
      }
  ...
  public long query(double c3_end, String c5_equal, boolean print, int caching)
      throws IOException, IndexNotExistedException {
    ArrayList<Range> list = new ArrayList<Range>();
    if (c3_end > 0) {
      list.add(new Range(Bytes.toBytes("orders"), Bytes.toBytes("f:c3"), null, CompareOp.NO_OP,
    ...
    if (IndexType.CCIndex == this.indexDesc.getIndexSpecification(list.get(index).getColumn())
      ...
      long stopTime = System.currentTimeMillis();
      System.out.println(
          "Time elapsed: " + (stopTime - startTime) + " ms, result count: " + result.length);
  ...
  public IndexResultScanner getScanner(IndexQuerySQL indexSql) throws Exception {
    if (!tableName.equals(indexSql.getTableName())) {
      throw new IllegalArgumentException(
          "IndexSQL isn't for this table! IndexSQL's table is " + indexSql.getTableName()
              + ", IndexTable's is " + tableName);
    }
    // check ranges
    ...
    for (Range[] temparray : range) {
      for (Range temprange : temparray) {
        ...
        if (temp == null || temp.length == 1 || !families.contains(temp[0])) {
          throw new IllegalArgumentException(
              "Column name is invalid! column name:" + Bytes.toString(temprange.getColumn()));
        }
      }
    }
    ...
    if (resultColumns != null && resultColumns.length != 0) {
      for (byte[] tempcolumn : resultColumns) {
        ...
        if (temp == null || !families.contains(temp[0])) {
          throw new IllegalArgumentException(
              "Column or family name is invalid! Column or family name:" + Bytes
                  .toString(tempcolumn));
        }
      }
    }
  ...
  private List<SingleScanner> doGetScanner(Range[] range, byte[][] resultColumns)
    ...
    if (flag <= -1) {
      ...
      if (range[flag].getEndValue() != null
          && range[flag].getEndType() == CompareOp.LESS_OR_EQUAL) {
        if (Bytes.compareTo(range[flag].getColumn(), IndexConstants.KEY) == 0) {
    ...
    LOG.debug("Scan table " + table.getName());
    ...
    if (maxScanners > 1) {
      ...
      LOG.debug(
          "Start parallel scan threads! MaxScanThreads=" + maxScanners + ", StartedScanThreads=" + (
              distance > maxScanners ?
                  maxScanners :
                  distance));
      ...
      if (distance > maxScanners) {
        ...
        for (int m = 0; m < maxScanners; m++) {
          ...
          LOG.debug(
              "Scan's StartRow=" + Bytes.toStringBinary(scan.getStartRow()) + " , StopRow=" + Bytes
                  .toStringBinary(scan.getStopRow()));
          indexScan.add(scan);
        }
    ...
    if (!containAll) {
      ...
      if (indexSpec.getIndexType() == IndexType.UDGIndex && indexSpec.getAdditionMap().size() != 0) {
        Map<byte[], Set<byte[]>> map = indexSpec.getAdditionMap();

        for (int i = 0; i < range.length; i++) {
          if (i != flag && Bytes.compareTo(range[i].getColumn(), IndexConstants.KEY) != 0) {
            byte[][] fq = KeyValue.parseColumn(range[i].getColumn());
            if (map.containsKey(fq[0]) && (map.get(fq[0]) == null || map.get(fq[0])
                .contains(fq[1]))) {
              if (range[i].getEndTs() != -1 || range[i].getStartTs() != -1) {
                TimeRangeFilter t =
                    new TimeRangeFilter(fq[0], fq[1], range[i].getStartTs(), range[i].getEndTs());
    ...
    if (flag <= -1 || Bytes.compareTo(range[flag].getColumn(), IndexConstants.KEY) == 0) {
      ...
      for (Scan tempscan : indexScan) {
        indexScanners.add(new IndexSingleScanner(tempscan, range, flag, resultColumns, conn, table,
  ...
  /**
   * Get all Tables, including main data table and index tables. For main data table, key of this
   * map is "key", for index tables, key is index column, e.g. "family:qualifier".
   *
   * @return
   */
  public Map<byte[], Table> getIndexTableMaps() {
  ...
  /**
   * Returns the maximum size in bytes of the write buffer for this Table.
  ...
  public void setWriteBufferSize(long writeBufferSize) throws IOException {
    ...
    for (Table table : indexTableMaps.values()) {
      table.setWriteBufferSize(writeBufferSize);
    }
    for (Table table : cctTableMaps.values()) {
      table.setWriteBufferSize(writeBufferSize);
    }
  }
  ...
  public void setScannerCaching(int scannerCaching) {
    ...
    for (Table table : indexTableMaps.values()) {
      if (table instanceof HTable) {
        ((HTable) table).setScannerCaching(scannerCaching);
      } else {
        LOG.warn("index table cannot setScannerCaching because it is not an HTable, it is: " + table
            .getClass().getName());
        return;
      }
    }
  ...
  public void setAutoFlush(boolean autoFlush) {
    ...
    for (Table table : indexTableMaps.values()) {
      if (table instanceof HTable) {
        ...
        LOG.warn("index table cannot set auto flush because it is not an HTable, it is: " + table
            .getClass().getName());
        return;
      }
    }
  ...
  public void flushCommits() throws IOException {
    for (Table table : indexTableMaps.values()) {
      if (table instanceof HTable) {
        ...
        LOG.warn("index table cannot flush commit because it is not an HTable, it is: " + table
            .getClass().getName());
        return;
      }
    }
